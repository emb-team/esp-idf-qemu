...
static void example_send_all_node_addr(struct k_work *work)
{
    ...
    if (atomic_test_and_clear_bit(fast_prov_srv_data->srv_flags, RELAY_PROXY_DISABLED)) {
        ...
        struct net_buf_simple *buf)
{
    ...
    case ESP_BLE_MESH_VND_MODEL_OP_FAST_PROV_INFO_SET: {
         ...
         */
        uint8_t status_unicast = 0, match_len = 0, match_val[16];
        ...
        uint8_t  flags         = (ctx_flags & BIT(3)) ? net_buf_simple_pull_u8(buf)   : (bt_mesh.iv_update << 1 | bt_mesh.sub[0].kr_flag);
        ...
        if (srv->state == STATE_ACTIVE) {
            ...
            case FAST_PROV_ACT_EXIT:
                ...
                if (!atomic_test_and_set_bit(srv->srv_flags, DISABLE_FAST_PROV_START)) {
    ...
    case ESP_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR: {
        ...
        if (atomic_test_and_clear_bit(srv->srv_flags, SEND_ALL_NODE_ADDR_START)) {
        ...
        esp_ble_mesh_msg_ctx_t *ctx)
{
    ...
    case ESP_BLE_MESH_VND_MODEL_OP_FAST_PROV_INFO_STATUS:
        if (err_code == 0 && srv->set_info && srv->set_info->set_succeed == true) {
            if (!atomic_test_and_set_bit(srv->srv_flags, RELAY_PROXY_DISABLED)) {
    ...
    case ESP_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_ACK:
        ...
        if (all_node_addr_cnt < srv->node_addr_cnt - 1) {
            if (!atomic_test_and_set_bit(srv->srv_flags, SEND_ALL_NODE_ADDR_START)) {
