...
static example_send_prov_node_addr_t node_addr_send = {
    .addr = NET_BUF_SIMPLE(NODE_ADDR_SEND_MAX_LEN),
    .send_succeed = true,
    .ack_received = true,
};

/* Self-provisioned node addresses that have been sent successfully */
uint16_t addr_already_sent[CONFIG_BT_MESH_MAX_PROV_NODES];
...
void example_send_self_prov_node_addr(struct k_work *work)
{
    ...
    atomic_test_and_clear_bit(&fast_prov_cli_flags, SEND_SELF_PROV_NODE_ADDR_START);
    ...
    if (node_addr_send.send_succeed == true) {
         ...
         */
        net_buf_simple_init(node_addr_send.addr, 0);
