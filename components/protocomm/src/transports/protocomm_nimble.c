...
static void transport_simple_ble_disconnect(struct ble_gap_event *event, void *arg)
{
    ...
    if (protoble_internal->pc_ble->sec &&
        ...
        ret =
            protoble_internal->pc_ble->sec->close_transport_session(event->disconnect.conn.conn_handle);
        if (ret != ESP_OK) {
...
static void transport_simple_ble_connect(struct ble_gap_event *event, void *arg)
{
    ...
    if (protoble_internal->pc_ble->sec &&
        ...
        ret =
            protoble_internal->pc_ble->sec->new_transport_session(event->connect.conn_handle);
        if (ret != ESP_OK) {
